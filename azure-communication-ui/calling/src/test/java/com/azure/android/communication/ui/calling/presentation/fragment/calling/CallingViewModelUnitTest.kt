...
internal class CallingViewModelUnitTest : ACSBaseTestCoroutine() {
    ...
    fun callingViewModel_onStateChange_when_stateIsBackground_then_doesNotCallChildViewModels() {
        runScopedTest {
            ...
            val mockParticipantGridViewModel =
                mock<ParticipantGridViewModel> {
                }

            val mockControlBarViewModel =
                mock<ControlBarViewModel> {
                    on { update(any(), any(), any(), any(), any()) } doAnswer { }
                }

            val mockConfirmLeaveOverlayViewModel = mock<LeaveConfirmViewModel> {}

            val mockLocalParticipantViewModel =
                mock<LocalParticipantViewModel> {
            ...
            val mockCallingViewModelProvider =
                mock<CallingViewModelFactory> {
    ...
    fun callingViewModel_onParticipantListChange_then_hideLobbyParticipantsOnGridAndParticipantList_ifCallStateIsNotConnected() {
        runScopedTest {
            ...
            newState.remoteParticipantState =
    ...
    private suspend fun TestScope.callViewOptionsTests(
        ...
        val mockLobbyHeaderViewModel =
            mock<LobbyHeaderViewModel> {
                on { update(any(), any(), any()) } doAnswer { }
            }
        val mockLobbyErrorHeaderViewModel =
            mock<LobbyErrorHeaderViewModel> {
        ...
        verify(
            ...
            argThat {
                    status ->
                status == CallingStatus.CONNECTED
            },
            argThat {
                    map ->
                map.size == lobbyParticipantCount
            },
            argThat { value -> value == true },
        )
    ...
    private fun getLocalUserState(
        localParticipantRole: CallCompositeInternalParticipantRole = CallCompositeInternalParticipantRole.PRESENTER,
    ) = LocalUserState(
        CameraState(
